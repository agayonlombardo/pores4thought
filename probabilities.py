...
os.makedirs('64_64_18', exist_ok=True)
...
for i in range(0, params['num_samples']):
    ...
    noise = torch.FloatTensor(1, params['nz'], 2, 2, 2).normal_(0, 1)
    ...
    tifffile.imsave('64_64_18/test_'+str(nW)+'_'+str(nH)+'__{0}.tif'.format(i), new_output)
        
    #probabilities map
    img_probs = output_probs.astype(np.uint8)
    tifffile.imsave('64_64_18/prob_map_'+str(nW)+'{0}.tif'.format(i), img_probs)
