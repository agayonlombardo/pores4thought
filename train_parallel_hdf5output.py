...
parser.add_argument('--nepochs', default=50, help='number of training epochs')
...
def save_hdf5(tensor, filename):
    ...
    with h5py.File(filename, 'w') as f:
        f.create_dataset('data', data=tensor, dtype="i8", compression="gzip")
###############################################
...
for epoch in range(opt.nepochs):
    for i, data in enumerate(dataloader, 0):
        ...
        if batches_done % opt.sample_interval == 0:
            #fake = netG(noise)
            save_hdf5(fake_data.data, 'images_2D/fake_{0}.hdf5'.format(batches_done))    
